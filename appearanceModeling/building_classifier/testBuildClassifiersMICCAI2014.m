...
mriSubjects = [ 50 51 52 53 54 56 57 58 59 ];%60 61 63 64 65 66]
%% Path Settings
clear m;
pathSettings;
%Tree Path Name
...
for s = mriSubjects 
    ...
    for r=settings.ribNumber
        ...
        groundTruthRibsPath{s}{r} = [subjectDataPath{s} 'Rib' side 'New' num2str(r)];
                    ...
                    groundTruthRibsPath{s}{r} = [subjectDataPath{s} 'Rib' side 'ExhNew' num2str(r)];
...
for s = mriSubjects
    tmp=load ( ['/home/sameig/N/Data/dataset' num2str(s) '/masterframes/resized_exhMaster7_unmasked_uncropped' ],'im');
    im{s}=tmp.im;
...
for s= mriSubjects 
	for rule=1:length(settings.rules)
        ...
        for r=ribNumbers
            ...
            pIndices = selectPointsR(tmpPoints,settings.rules{rule},settings,1);
...
%         prob=vote(:,2)/numTrees;
        sensitivity(s,rule) =  sum(predicted_categories)/size(featuresRibs{rule}{s},2);

%         sensitivity(s,rule) = 1- sum(prob<0.5)/size(featuresRibs{rule}{s},2);
         
        test_patches = [featuresNonRibs{rule}{s} ]';


        [predicted_categories, vote] = classRF_predict(test_patches,treeM{s}{rule});
%         prob=vote(:,2)/numTrees;
        specificity(s,rule) = 1- sum(predicted_categories)/size(featuresNonRibs{rule}{s},2);

%         specificity(s,rule) = 1- sum(prob>0.5)/size(featuresNonRibs{rule}{s},2);
...
sensitivity(mriSubjects,:)
specificity(mriSubjects,:)
mean(mean(sensitivity(mriSubjects,:)))
mean(mean(specificity(mriSubjects,:)))
% mean(cell2mat(accuracy{rule}(mriSubjects)))
%%
% end
% ribP{rule}{s}
