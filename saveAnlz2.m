...
function saveAnlz2(im,par,fname,doShiftPlusOne,mode)
...
im_ = (im - mi ) *  255 /( prctile(im(:),99) - mi ) +  mi;
% im_ = (im - mi ) *  255 /( ma - mi ) +  mi;
im = limitVals(im_,0,255);
...
if (nSlices==1)
  avwout.img = uint16(permute(im,[2 1]));
    ...
    if mode ==3
        avwout.img = flip(permute(im,[2 1 3]),3);
    end
    if mode ==2
