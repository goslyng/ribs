...
ribcageWOAngle=[reshape(ribShapeModel.proj(:,1),[],length(ctSubjects))',... 
        reshape(ribShapeModel.proj(:,2),[],length(ctSubjects))' ,... 
        lenMatrix',...reshape(len,[],numel(ctSubjects))',... 
         ];
    
meanRibCageCTWOAngle =repmat(mean(ribcageWOAngle),size(ribcageWOAngle,1),1);
ribcageCTWOAngle = ribcageWOAngle - meanRibCageCTWOAngle ; 
coefs = 100./ mean(abs(ribcageCTWOAngle));

ribcageCTWOAngle = ribcageCTWOAngle.*repmat(coefs,size(ribcageCT,1),1);
ribcageCTWOAngle = ribcageCTWOAngle+meanRibCageCTWOAngle;

ribcageWOAngleModel = pcaModeling (ribcageCTWOAngle);
ribcageWOAngleModel.coefs  = coefs;
ribcageWOAngleModel.ribs = (testRibs+1)/2;
ribcageWOAngleModel.meanLen = meanLen;
