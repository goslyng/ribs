...
ang_proj = pcaProject(angleModel, angleParams, nCompsAngle);
testRibs = 7:10;

ribCageLengthModel = ribModel.ribCageLengthModel;
lenProjected = ribModel.ribCageLengthModel.proj;
lengthParams(:,testRibs) = pcaProject(ribCageLengthModel, lenProjected, nCompsLength);
ribShapeModel = ribModel.ribShapeModel;
compModel = ribModel.ribShapeCoeffsModel;

%%
for i=1:length(ribModel.ribShapeCoeffsModel)
    compProj{i} = ribModel.ribShapeCoeffsModel{i}.proj;
end
...
for s=ctSubjects
     ...
     ang_rec(1,s,testRibs)=ang_proj(s,1:3:end);%reshape(mean(ang_mat),3,[]) + squeeze(repmat(mean(ang(:,s,5:end),3),[1 1 6]));
     ang_rec(2,s,testRibs)=ang_proj(s,2:3:end);%reshape(mean(ang_mat),3,[]) + squeeze(repmat(mean(ang(:,s,5:end),3),[1 1 6]));
     ang_rec(3,s,testRibs)=ang_proj(s,3:3:end);%reshape(mean(ang_mat),3,[]) + squeeze(repmat(mean(ang(:,s,5:end),3),[1 1 6]));
end
