...
function [T, ribs,fval] =  fitnessFunction3( T0, ribShapeModel,heatMaps , settings,LB,UB)
    ...
    LB_(7:8) = LB(7:8) + ribShapeParams;
    UB_(7:8) = UB(7:8) + ribShapeParams;
    
    LB_(9) = LB(9) + lenProjected;
    UB_(9) = UB(9) + lenProjected;
    
    
    
    [T,fval] = ga(@nestedfun,length(LB),[],[],[],[],LB_,UB_);
    
    Pp_ = pcaProject(ribShapeModel,T(7:8),2);
    Pp_ = Pp_*T(9);
    ...
    function ribCost = nestedfun(T)
        ...
        Pp = pcaProject(ribShapeModel,T(7:8),2);
        Pp = Pp*T(9);
...
%         R = findEuler(angles(1),angles(2),angles(3),2);
        R = findEuler(angles(1),angles(2),angles(3));
