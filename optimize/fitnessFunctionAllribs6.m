...
function [T, ribs,fval] =  fitnessFunctionAllribs6(LB,UB, firstPts, ribcageWOModel,ribShapeModel,angleModel,heatMaps , settings)



    options = gaoptimset('PopulationSize',1000);
    nRibs = length(ribcageWOModel.ribs);
    [T,fval] = ga(@nestedfun,length(LB),[],[],[],[],LB,UB,[],options);
    
    %%
    p_ = buildRibsParamsAngle(T,firstPts,settings,ribcageWOModel,ribShapeModel,angleModel);
...
%         ribs_ = buildRibsParams(T,firstPts,settings,ribcageModel,ribShapeModel,settings.nStd);
        ribs_ = buildRibsParamsAngle(T,firstPts,settings,ribcageWOModel,ribShapeModel,angleModel);
        for r=1:nRibs
