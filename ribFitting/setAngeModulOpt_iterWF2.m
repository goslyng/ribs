...
function mean_er = setAngeModulOpt_iterWF2(m,part1,inh)
...
addpath(paths.codePathRibs)
addpath([paths.codePathRibs '/ribFitting/auxiliary']);
loadAndPrepareData;
...
resultPath0 = [paths.rootPath 'Ribs/ang_' 'scale' num2str(scale) '_sigma'  num2str(10*settings.hw_sigma(1),'%02d')  '_' num2str(m)];

for i=1:3
    resultPath{i} = [paths.rootPath 'Ribs/res_firstP_'  '_step' num2str(i) '_sigma'  num2str(10*settings.hw_sigma(i),'%02d')  '_' num2str(m)];
end
% resultPath2Old = [rootPath 'Ribs/ang_' 'scale' num2str(scale) '_sigma'  num2str(10*settings.hw_sigma2,'%02d')  '_' num2str(m)];
if inh
    resultPath1 = [paths.rootPath 'Ribs/ang_inh_scale' num2str(scale) '_sigma'  num2str(10*settings.hw_sigma(2),'%02d')  '_' num2str(m)];
    resultPath2 = [paths.rootPath 'Ribs/ang_inh_scale' num2str(scale) '_sigma'  num2str(10*settings.hw_sigma(3),'%02d')  '_' num2str(m)];
end
if (m==59 || m==60 || m==550)
...
if part1==1
    ...
    heatMaps = loadAllHeatMaps(paths.heatMapPathRule,m,settings.postFix,scale,paths.im_size_path,settings,settings.hw_sigma0);
...
for part=2:part1
   ...
   [~,b]=sort(mean(cost(:,settings.ribNumber),2));
    ...
    save([resultPath{part} 'test'],'ang','cost','offset_indx');
end
